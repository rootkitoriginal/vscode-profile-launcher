...
export class GitHubService {
  ...
  public async listIssues(owner: string, repo: string, state?: IssueState): Promise<GitHubIssue[]> {
    if (!this.octokit) {
      await this.initialize();
    }
    if (!this.octokit) {
  ...
  public async listRepositories(owner: string): Promise<Array<{ name: string; fullName: string; description: string; defaultBranch: string }>> {
    if (!this.octokit) {
      await this.initialize();
    }
