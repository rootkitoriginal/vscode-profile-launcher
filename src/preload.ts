...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    getProfile: (id: number): Promise<Profile | null> => 
        ipcRenderer.invoke('get-profile', id),
    ...
    generateCodeTemplate: (language: string, projectName: string, description?: string, preferredProvider?: 'gemini' | 'openai', preferredModel?: string): Promise<any> => 
        ipcRenderer.invoke('generate-code-template', language, projectName, description, preferredProvider, preferredModel),
    
    generateCode: (request: any): Promise<any> => 
        ipcRenderer.invoke('generate-code', request),
    
    getAvailableProviders: (): Promise<Array<{ name: 'gemini' | 'openai', configured: boolean }>> => 
        ipcRenderer.invoke('get-available-providers'),

    getProfilePaths: (profileName: string): Promise<{ baseDir: string; dataDir: string; extensionsDir: string }> => 
    ...
    githubCreateIssue: (owner: string, repo: string, title: string, body: string, labels?: string[]) => 
    ...
    isGitHubConfigured: (): Promise<boolean> => 
        ipcRenderer.invoke('is-github-configured'),
    ...
    githubListRepos: (owner: string): Promise<Array<{ name: string; fullName: string; description: string; defaultBranch: string }>> => 
...
declare global {
    interface Window {
        electronAPI: {
            ...
            updateGitHubToken: (token: string) => Promise<boolean>;
            getGitHubToken: () => Promise<string>;
            githubListIssues: (owner: string, repo: string, state?: 'open' | 'closed') => Promise<any>;
            githubCreateIssue: (owner: string, repo: string, title: string, body: string, labels?: string[]) => Promise<any>;
